...
- **Fotos**: em 2026-08-04 o cliente entregou as **primeiras fotos reais do espaço**, que substituíram o stock em todos os sítios onde há foto do sítio — Sobre (`sala-restaurante.jpg`, a sala com o teto de madeira e a parede de pedra) e Galeria (`placa-entrada.jpg`, a placa com o nome novo; `esplanada.jpg`, as mesas com os chapéus; `entrada-restaurante.jpg`, a fachada vista do jardim). Continuam de stock **só os três slots de comida** — `peixe-grelhado`, `prato-do-dia` e `sobremesa-casa` (Unsplash/Pexels, licença livre para uso comercial, sem atribuição): **não são pratos do Mira Mar** e continuam à espera de fotos reais.
  - **O Hero é a `praia-angeiras.jpg` — a praia de Angeiras, decisão do cliente.** Chegou a estar lá a fachada durante uma iteração e foi **rejeitado**: a praia é a imagem que o cliente escolheu para a primeira dobra e é a que fica. A fachada não se perdeu — passou para o sexto slot da Galeria, no lugar de uma `vista-mar.jpg` de stock que, com a praia no hero, era repetição (o ficheiro saiu do repo; recupera-se do git se um dia fizer falta).
  - ⚠️ **A `praia-angeiras.jpg` não tem licença conhecida.** Foi fornecida pelo cliente a partir de uma pesquisa na web. É a única coisa por licenciar no repo e **tem de se confirmar a origem com o cliente antes de publicar a sério**. É também por isso que o cartão de partilha usa o peixe grelhado e não esta foto — não vale a pena espalhá-la mais.
  ...
  - ⚠️ **Resolução**: as fotos reais do espaço chegaram a 768×1024 (redimensionadas por partilha de telemóvel) e a praia a 1024×680. Chega para a Galeria e para o Sobre, mas o Hero é servido a `sizes="100vw"` e num portátil estica 1024 px para ~1440: **pedir ao cliente o original da praia** e trocar o ficheiro é a melhoria de qualidade mais barata que este site tem por fazer.
  - As fotos do espaço são retrato 3:4, a praia é paisagem 3:2, e nenhuma vem cortada do repo: o enquadramento de cada sítio é feito pelo `object-cover` conforme o ecrã. Ao trocar a foto do Hero, confirmar que a faixa central continua clara — é o que faz o `.titulo-destaque` ler (o halo e o véu radial foram calibrados precisamente para esta praia, que é clara e ocupada).
...
- **Cartão de partilha** (`src/app/[locale]/opengraph-image.tsx`) — gerado por código, um por língua, e **em JPEG**: em PNG dava 1,8 MB e o WhatsApp ignora pré-visualizações grandes. Usa a foto do **peixe grelhado e não a do hero**, por duas razões que se reforçam: a foto do hero é a praia sem licença conhecida (ver a secção das fotos) e republicá-la num cartão de partilha só a espalha mais; e num cartão de WhatsApp a comida convida mais do que uma paisagem. Se um dia a licença da praia ficar resolvida e chegar o original em resolução alta, vale a pena comparar os dois. A Fraunces está versionada em `assets/` — o `next/font` não a serve ao Satori. Duas armadilhas do Satori já pagas: não entende o atalho `inset` nem o atalho `background` (com eles as camadas do véu saem invisíveis).
