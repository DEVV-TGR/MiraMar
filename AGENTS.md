...
- **Duas ementas**, ambas páginas reais do site e traduzidas nas 4 línguas: a **carta do restaurante** (`/{locale}/ementa`, com botão "Descarregar PDF" — só português, ver abaixo) e o **take away** (`/{locale}/take-away`). QR code das mesas e o CTA "Ver Ementa" apontam para a carta.
...
- As ementas (`src/data/menu.json`, `src/data/takeaway.json`) seguem o mesmo princípio: cada `nome`/`descricao` é um objeto `{ pt, en, fr, es }`; `preco` é universal (€, não convertido por idioma).
...
### As duas ementas: carta do restaurante + take away
...
- `src/components/ementa/ListaEmenta.tsx` renderiza as duas (recebe um `Menu`), e `AlternadorEmenta.tsx` são as tabs "Carta Restaurante | Take Away" no topo de ambas. **Páginas separadas de propósito** — são duas coisas diferentes e não se misturam numa lista só; as tabs é que tornam a passagem imediata. O Hero tem os dois botões, o `Header` ficou inalterado (acrescentar mais um link desequilibrava a grelha de 3 colunas que mantém o logo centrado).
- `npm run menu:pdf` gera `public/mira-mar-menu.pdf` via `@react-pdf/renderer` (`scripts/generate-menu-pdf.mjs`) — **só a carta do restaurante e só em português** (decisão do cliente: manter o PDF como download opcional, não vale a pena gerar 4 PDFs traduzidos). O take away vive só na web. Uma coluna, não duas: com cinco menus, duas colunas deixavam meia página em branco.
